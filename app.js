firebase.initializeApp({
    messagingSenderId: '748158835814'
});
...
function setNotificationDemoBody() {
    if (input_body.val().search(/^Посмотри, что ученые обнаружили сегодня в \d\d:\d\d$/i) !== -1) {
        var now = new Date();
        input_body.val('Посмотри, что ученые обнаружили сегодня в ' + now.getHours() + ':' + addZero(now.getMinutes()));
    } else {
...
function sendNotification(notification) {
    var key = 'BGhVlZcepuYegPgmoF_MGjotKpd2emuAiLwXtBo_mjM7-b5Y-VKvBqoYGODw7JZXFaRe97POKLpY080ciWBxmDE';
